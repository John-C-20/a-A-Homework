...
def andrews_films_and_leads
  ...
  WHERE castings.ord = 1 AND movies.title IN
    ...
    JOIN castings ON id = movie_id 
    JOIN actors ON actor_id = actors.id 
    WHERE actors.name = 'Julie Andrews') -- movies with Julie Andrews
  SQL
end
...
def prolific_actors
  ...
  WHERE name IN 
    ...
    GROUP BY actors.name, castings.ord 
    HAVING castings.ord = 1 AND COUNT(*) > 14) -- list of actors where ord is 1 and has at least 15 rows
  ORDER BY name
  SQL
end
