...
def casablanca_cast
  ...
  execute(<<-SQL)
  SELECT name FROM actors JOIN castings on id = actor_id WHERE movie_id = 27 
  SQL
end
...
def alien_cast
  ...
  execute(<<-SQL)
  SELECT name FROM actors JOIN castings on id = actor_id 
  JOIN movies ON movie_id = movies.id WHERE movies.title = 'Alien'
  SQL
end
